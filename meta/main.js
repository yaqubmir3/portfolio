...
function processCommits(data) {
  return d3
    ...
    .map(([commit, lines]) => {
      ...
      let ret = {
        id: commit,
        url: 'https://github.com/yaqubmir3/portfolio/commit/' + commit,
        author,
        date,
        time,
        timezone,
...
function renderCommitInfo(data, commits) {
  ...
  const avgLen = d3.mean(
    d3.rollups(data, (v) => d3.max(v, (v) => v.line), (d) => d.file),
    (d) => d[1]
  );
  dl.append('dd').text(Math.round(avgLen) + ' lines');
...
function updateTooltipPosition(event) {
  ...
  if (left + tooltipWidth > window.innerWidth) left = event.clientX - tooltipWidth - 12;
